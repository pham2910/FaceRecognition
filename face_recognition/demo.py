...
def recog_face_gender_age():
    ...
    while cv2.waitKey(1) < 0:
        ...
        for(x,y,w,h) in faces:
            ...
            if(profile!=None):
                cv2.putText(frameFace, "Name: " + str(profile[1]), (x+20, y+h+20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 1, cv2.LINE_AA)
...
tk.Label(recog_face_gender_ageFrame,text="Face Recognition",font=("Arial", 60),bg="white").grid(row=1,column=2,columnspan=5)
...
tk.Label(RegisterFrame,text="Register",font=("Arial",60),bg="white").grid(row=1,column=2)
tk.Label(RegisterFrame,text="ID",font=("Arial",30),bg="white").grid(row=2,column=1)
